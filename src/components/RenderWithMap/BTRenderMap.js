...
export default class BTRenderMap extends Component {
        ...
        const arrFilm = this.dataFilm.map((film, index) => {
            return <div key={index} className="col-4">
                <div className="card text-white bg-primary">
                    {/* Cách 1: */}
                    {/* <img width="150" height="300" className="card-img-top w-100" src={film.hinhAnh} alt={film.hinhAnh} /> */}
                    {/* Cách 2: */}
                    <div style={{
                    ...
                    <div className="card-body">
                        <h4 className="card-title" key={index}>Tên film : {film.tenPhim}</h4>
                        {/* <h4 className="card-title" style={{ height=50 }} >Tên film :{film.tenPhim.length > 50 ? <p>{film.tenPhim.sub(0, 100)} ...</p> : <p>{film.tenPhim}</p>}</h4> */}
                        ...
                        <p className="card-text">Mã nhóm : {film.maPhim}</p>
                        <p className="card-text">Ngày khởi chiếu : {film.ngayKhoiChieu}</p>
                        <p className="card-text">Đánh giá : {film.danhGia}</p>
                    </div>
                </div>
            </div >
        })
    ...
    //Viết hàm hiển thị danh sách film dưới dạng div card bootstrap
    render() {
        return (
